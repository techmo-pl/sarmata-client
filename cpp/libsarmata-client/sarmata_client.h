...
public:
    ...
    DefineGrammarResponse DefineGrammar(const SarmataClientConfig& config) const;

    std::vector<RecognizeResponse> Recognize(const SarmataClientConfig& config, unsigned int audio_sample_rate_hz, const std::string& audio_byte_content) const;
