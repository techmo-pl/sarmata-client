...
int defineGrammar(const po::variables_map & userOptions) {
    ...
    session.PreDefineGrammar(userOptions["grammar-name"].as<std::string>(), grammar);
...
po::options_description CreateOptionsDescription(void) {
    ...
    optionsDescription.add_options()
            ...
            ("no-match-threshold", po::value<double>()->default_value(0.2),
             "Confidence acceptance threshold.")
            ("--no-input-timeout", po::value<int>()->default_value(5000),
             "MRCPv2 No-Input-Timeout in milliseconds.")
            ("--recognition-timeout", po::value<int>()->default_value(10000),
             "MRCPv2 Recognition-Timeout in milliseconds.")
            ("--speech-complete-timeout", po::value<int>()->default_value(500),
             "MRCPv2 Speech-Complete-Timeout in milliseconds.")
            ("--speech-incomplete-timeout", po::value<int>()->default_value(3000),
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (userOptions.count("wav-path")) {
            if (not (userOptions.count("grammar") || userOptions.count("grammar-name"))) {//bad usage
                std::cerr << "Usage (Recognize): " << argv[0] << " --service-address host:address --wav-path /path/to/audio.wav --grammar-name name"
                        "\n   or " << argv[0] << " --service-address host:address --wav-path /path/to/audio.wav --grammar data" << std::endl;
                return 1;
            }

            const auto wavPath = userOptions["wav-path"].as<std::string>();
            if (not fileExists(wavPath)) {
                std::cerr << "Wave file does not exist: " << wavPath << std::endl;
                return 1;
            }

            const auto grammarName = userOptions.count("grammar-name") ? userOptions["grammar-name"].as<std::string>() : "";
            ...
            if (grammarName.empty() && grammar.empty()) {
